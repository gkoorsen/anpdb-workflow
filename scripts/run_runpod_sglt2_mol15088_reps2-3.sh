...
for rep in 2 3; do
  config="configs/md/production/sglt2_mol15088_100ns_rep${rep}.toml"
  run_dir="md_runs/production/sglt2_mol15088/rep${rep}"
  required=(
...
if [[ "${check_only}" == true ]]; then
  echo "RunPod preflight passed for SGLT2 Mol_15088 replicates 2-3."
  exit 0
fi
...
for rep in 2 3; do
  ...
  if [[ -s "${run_dir}/run_manifest.json" && -s "${run_dir}/final_state.xml" ]]; then
    echo "SKIP completed SGLT2 Mol_15088 replicate ${rep}"
    continue
  fi
  ...
  if [[ -s "${run_dir}/production.dcd" || -s "${run_dir}/production.log" ]]; then
    ...
    mode="--resume"
    echo "Resuming SGLT2 Mol_15088 replicate ${rep} from its RunPod checkpoint"
  else
    mode="--resume-state"
    echo "Starting SGLT2 Mol_15088 replicate ${rep} from portable equilibrated_state.xml"
  fi
...
echo "Completed SGLT2 Mol_15088 replicates 2-3"
