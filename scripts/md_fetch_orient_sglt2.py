"""Fetch OPM-oriented 7VSI coordinates and orient the SGLT2 ligand pose.

The production SGLT2 configs need the receptor in the membrane frame. The
docked ligand pose was generated in the original PDB coordinate frame, so the
same rigid-body transform must be applied to the ligand after fetching the
oriented receptor.

Usage:
  python scripts/md_fetch_orient_sglt2.py
  python scripts/md_fetch_orient_sglt2.py --force
"""
...
DEFAULT_URL = "https://encompass.ninds.nih.gov/webfiles/database/selection/whole_structs/7vsi_enc.pdb"
DEFAULT_RAW = ROOT / "data" / "md_inputs" / "receptors" / "7VSI_encompass_raw.pdb"
DEFAULT_REFERENCE = ROOT / "data" / "md_inputs" / "receptors" / "7VSI_clean_unoriented_reference.pdb"
FALLBACK_REFERENCE = ROOT / "output" / "docking" / "receptors" / "7VSI_clean.pdb"
DEFAULT_RECEPTOR_OUT = ROOT / "data" / "md_inputs" / "receptors" / "7VSI_opm_oriented_clean.pdb"
DEFAULT_LIGAND_IN = ROOT / "data" / "md_inputs" / "poses" / "Mol_13144_7VSI_out.pdbqt"
DEFAULT_LIGAND_OUT = ROOT / "data" / "md_inputs" / "poses" / "Mol_13144_7VSI_opm_oriented_out.pdbqt"
DEFAULT_MANIFEST = ROOT / "data" / "md_inputs" / "sglt2_orientation_manifest.json"
...
def matched_ca(reference_path: Path, oriented_path: Path, chain: str) -> tuple[np.ndarray, np.ndarray, int]:
    ...
    if len(keys) < 10:
        fail(f"Only found {len(keys)} common CA atoms between reference and oriented 7VSI.")
...
def write_clean_receptor(raw_path: Path, output_path: Path, chain: str, force: bool, source_url: str) -> int:
    ...
    with output_path.open("w") as handle:
        handle.write("REMARK  Generated by scripts/md_fetch_orient_sglt2.py\n")
        handle.write(f"REMARK  Source: {source_url}\n")
        handle.write("REMARK  Kept ATOM records for SGLT2 chain A; removed MAP17 and OPM dummy atoms.\n")
...
def main() -> int:
    ...
    if not reference.exists():
        fail(f"Missing reference 7VSI receptor: {args.reference}")
